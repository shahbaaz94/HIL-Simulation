...
void pidctrl_traction(float t, float dt, float y, float &u);
void pidctrl_speed(float t, float dt, float y, float &u);
void pidctrl_brake(float t, float dt, float y, float &u);
...
{		
  ...
  float y1p=0.0,y2p=0.0,a1,a2;
    ...
    if (t < 30) pidctrl_traction(t, dt, slipratio,u1);

//    if (wf < 795) pidctrl_traction(t, dt, slipratio,u1); // launch control and traction control
//    else pidctrl_speed (t, dt, a1+a2,u1); // speed control

    //if (t > 30)  pidctrl_brake(t, dt, a1 + a2, u1);
//    if (t > 20) {
//      if(slipratio < 0.2)  pidctrl_brake(t, dt, a1 + a2, u1);
//      else  pidctrl_traction(t, dt, slipratio,u1);
//    }
...
void pidctrl_traction(float t, float dt, float y, float &u){
	...
	kp = 320.0; //kp = 1 --- wf = 50; kp = 10 --- wf = 500; kp = 16 --- wf = 800 propotional just like mu  
...
void pidctrl_speed(float t, float dt, float y, float &u){
  ...
  // this pid is for cruise control
  kp = 50.0; //kp = 1 --- wf = 50; kp = 10 --- wf = 500; kp = 16 --- wf = 800 propotional just like mu  
  // but if kp > 16 traction control effect will be worse but can improve time response
  ki = 0.0;  // traction control will be worse while improve time response, somehow prohibit mu to control speed
  kd = 0.0;  // will make traction control impossible
